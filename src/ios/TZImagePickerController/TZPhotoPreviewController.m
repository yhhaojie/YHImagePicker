...
#warning update
        if (_tzImagePickerVc.selectedModels.count>0) {
            TZAssetModel *oldModel = [_tzImagePickerVc.selectedModels objectAtIndex:0];
            for (TZAssetModel *eModel in _models) {
                if ([[eModel.asset valueForKey:@"uuid"] isEqualToString:[oldModel.asset valueForKey:@"uuid"]]) {
                    eModel.isSelected = NO;
                    break;
                }
                /*
                if ([oldModel isEqual:eModel]) {
                    eModel.isSelected = NO;
                    break;
                }*/
            }
            [_tzImagePickerVc.selectedModels removeObjectAtIndex:0];
        }

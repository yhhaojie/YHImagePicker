...
#warning update
            if (_tempIndexRow >-1) {
                // 已经单选过
                [tzImagePickerVc.selectedModels removeObjectAtIndex:0];
                
                TZAssetCell *oldCell = (TZAssetCell*)[collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:_tempIndexRow inSection:0]];
                oldCell.selectImageView.image = isSelected ? [UIImage imageNamedFromMyBundle:@"photo_sel_photoPickerVc"] : [UIImage imageNamedFromMyBundle:@"photo_def_photoPickerVc"];
                oldCell.selectPhotoButton.selected = NO;
                oldCell.model.isSelected = NO;
                
                ((TZAssetModel*)_models[_tempIndexRow]).isSelected = NO;
            } else if (tzImagePickerVc.selectedModels.count>0){
                [tzImagePickerVc.selectedModels removeObjectAtIndex:0];
            }
            // 还未单选
...
#warning update
        ...
        if (tzImagePickerVc.imageUuid.length>0) {
            if ([[model.asset valueForKey:@"uuid"] isEqualToString:tzImagePickerVc.imageUuid]) {

                TZAssetModel *tzAssetModel = [TZAssetModel modelWithAsset:model.asset type:[[TZImageManager manager] getAssetType:model.asset]];
                model.isSelected = YES;
                
                tzImagePickerVc.selectedAssets = [NSMutableArray arrayWithObject:model.asset];
                tzImagePickerVc.selectedModels = [NSMutableArray arrayWithObject:tzAssetModel];
